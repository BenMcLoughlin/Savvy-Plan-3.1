...
const getSavingsAreaMinMax = dataObject => {
  const savingsByYear = Object.values(dataObject).map((d:any) => d.principle + d.interest)
  const max = d3.max(savingsByYear)
  return max + 10000
}
 ...
 **/
export const getMax = (className, data, dataObject, state) => {
