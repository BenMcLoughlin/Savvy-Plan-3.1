...
export const numberOfStreams = (state: I.state, user: I.user, streamType: I.streamType): number => {
  return Object.values(state.streams_reducer as I.streams_reducer).filter((d: I.stream) => d.owner === user && d.streamType === streamType).length
}

export const streams = (state: I.state, user: I.user, streamType: I.streamType): I.stream[] => {
  return Object.values(state.streams_reducer as I.streams_reducer).filter((d: I.stream) => d.owner === user && d.streamType === streamType)
}

export const removeMostRecentStream = (state: I.state, user: I.user, remove: I.remove, set: I.set, streamType: I.streamType): void => {
  const streams: I.stream[] = Object.values(state.streams_reducer as I.streams_reducer)
