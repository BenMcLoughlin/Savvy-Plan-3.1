...
export const getThisYearsIncome = (stream: I.incomeStream, year: I.year) => {
  ...
  const value = _.range(1, periods+1).map((n) => {
    if (year >= stream.in[n].start && year < stream.in[n].end) return stream.in[n].value;
    else return 0;
  });
...
export const getStreams = (state: any, user: string, year: I.year, query: string) => {
  const {main_reducer} = state
  const userIncomeStreams_array = Object.values(main_reducer)
    .filter((d: any) => (d.streamType === "income" || d.streamType === "savings") && d.owner === user)
