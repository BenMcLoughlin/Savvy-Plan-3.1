...
import {formatIncomeName} from "view/charts/createChartFunctions/chartHelpers"
  ...
  return `
      <div style="${tooltipWrapper}">
        ...
        <div style="${displayBox}">
          <div style="${displayValue1}">${round(data.value)}</div>
          <div style="${subTitle}">Ideal Value at age 65</div>
        </div>
        <div style="${centerTitle}">So it can provide</div>
        <div style="${displayBox}">
          <div style="${displayValue1}">${round(data.income)}</div>
