...
import { formatIncomeName } from "view/charts/drawCharts/chartHelpers"
...
const subTitle = `
  ...
  text-align: center;
`
export const cppCenterTooltipHtml = (data, selectedAge, state) => {
      ...
      <div style="${tooltipWrapper}">
        ...
        <div style="${row}">
          <div style="${displayBox}">
            <div style="${displayValue1}">${round(d.value)}</div>
            <div style="${subTitle}">Annual Income</div>
          </div>
          <div style="${displayBox}">
            <div style="${displayValue1}">${round(d.value / 12)}</div>
...
export const axisHtml = (d, value) => {
  return `
      <div style="${axisWrapper}">
        ...
        <div style="${axisBox}">
          <div style="${axisValue}">${round(d.value)}</div>
        </div>
      </div>
                                    `
}
