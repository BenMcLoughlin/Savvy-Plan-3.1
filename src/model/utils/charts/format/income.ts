...
function afterTaxIncomeDetails(fnfnState, user, year) {
  const { birthYear, cppStartAge, oasStartAge, tfsaStartAge, rrspStartAge } = fnfnState.user_reducer[
      user
    ],
  ...
  return {
    ...
    maxRrsp: -fin.PMT(
      ...
      Object.entries(inc).reduce(
        (a, [k, v]) =>
          a +
          (+k > startWork && +k < birthYear + rrspStartAge
            ? checkMax(v[user].cppEligible * 0.18, k) + a * r1
            : 0),
        0
      )
    ),
...
function getTargetIncome(fnfnState, user, year, taxableInc) {
  ...
  const returnValue = {
    rrsp:
      year >= birthYear + rrspStartAge
        ? rrspPerc * totalDiff < maxRrsp
          ? rrspPerc * totalDiff
          : maxRrsp
        : 0,
...
const targetWithdrawals = (fnfnState, user) => {
  ...
  const returnValue = ["tfsa", "rrsp", "nreg"].reduce(
    (a, n) => ((a[`${n}Inc`] = forcast[startIncAge[n] + 5][user].income[n]), a),
    {}
  )
  return returnValue
}
