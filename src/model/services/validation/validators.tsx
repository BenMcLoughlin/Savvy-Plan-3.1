...
export const isValidChildYear = (year: number): boolean => year > 1972 && year < 2030
...
export const validateText = (name: string, value: string, formData?: I.formData): I.validText => {
  const error =
    name === "email"
      ...
      ? !isValidYear(+value)
      : /BirthYear/.test(name)
      ? !isValidChildYear(+value)
      : false

  return { isError: error, text: errorText[name] }
}

export const validateNext = (value: I.a, data: I.objects): boolean => {
  ...
  switch (component) {
    case "null":
      return true
    case "TextInput":
      return name === "year" ? isValidYear(value) : value.length > 2
