...
import { createStream, newIncomeStream } from 'services/ui_functions'

export const onboard_data = (state: any, setValue_action: any, progress: number) => {
  const { user_reducer, main_reducer, ui_reducer } = state

  const { maritalStatus, hasChildren, birthYear, firstName } = user_reducer
  ...
  Object.values(main_reducer)
    ...
    .map((instance: any) => {
      ...
      const incomeData = createIncomeArray(instance, setValue_action, state) //creates an array for each income stream, enabling the user to change individual details in the wizard
...
//     const newStream = newIncomeStream('red', 'Employment', 'Wal Mart Income', 0, true, 2010, 1000, 2029)
...
//       .map((instance: any) => {
//         //looks at all the income streams listed in the main reducer
//         const incomeData = createIncomeArray(instance.id, setValue_action, state) //creates an array for each income stream, enabling the user to change individual details in the wizard
//         incomeData.map((d: any, i: number) => {
//           //maps through the array and pushes the contents to the main array that controls the wizard
//           array.push(d)
//         })
