...
import { getOnboardQuestions } from "services/questions/questionCreators"
import { getCommentsToShowUser } from "services/questions/show"
...
export const onboardQuestions_data = (data: any, state: any, set: any, progress: number, remove: any) => {
  ...
  const { maritalStatus, user1, haveChildren, numberOfChildren, user1Gender, user1BirthYear, user2BirthYear, user1Name, user2Name, ownHome, hasUnsecuredDebt } = user_reducer
  ...
  const ask = getOnboardQuestions(q, state, set)
  const show = getCommentsToShowUser(q, state, set)

  show.everyone.onboardIntro()
  ask.for.user1.name()
  ask.for.user1.birthYear()
  ask.for.user1.gender()
  if (user1.isMarried) {
     ask.for.user2.name()
     ask.for.user2.birthYear()
     ask.for.user2.gender()
   }
  ask.if.theyHaveChildren()
  if (user1.hasChildren) {
      ask.for.their.numberOfChildren()
    }
  ask.for.user1.income.details()
  if (user1.isMarried) {
    ask.for.user2.income.details()
  }
  ask.for.user1.savingsDetails()
  if (user1.isMarried) {
    ask.for.user2.savingsDetails()
  }
  ...
  if (maritalStatus === "married" || maritalStatus === "common-law") {
    _questions.push({
      ...
      handleChange: () => {
        ...
        createStreamV2("income", "in", "user2", "taxable", set, state)
  ...
  _questions.push({
    ...
    handleChange: (selected, d: any) => {
      if (!selected && d.label !== "none") {
        ...
        createStreamV2("savings", "out", "user1", d.reg.toLowerCase(), set, state)
  ...
  if (maritalStatus === "married" || maritalStatus === "common-law") {
    _questions.push({
      ...
      handleChange: (selected, d: any) => {
        if (!selected && d.label !== "none") {
          // check if the item doesnt already exist, or its not none, and will then create a new income st
          createStreamV2("savings", "out", "user2", d.reg.toLowerCase(), set, state)
        } //checks if there is no currently selected version, if so it adds a new one, prevents adding mulitple with many clicks
        if (selected) {
  ...
  _questions.push({
    ...
    handleChange: () => {
      ...
      createStreamV2("property", "out", "user2", "", set, state)
    },
  ...
  _questions.push({
    ...
    handleChange: () => {
      ...
      createStreamV2("debt", "out", "user1", "", set, state)
    },
  ...
  _questions.push({
    ...
    handleChange: () => {
      set("hasUnsecuredDebt", "user_reducer", true)
      createStreamV2("debt", "out", "user2", "", set, state)
    },
    handleChange2: () => set("hasUnsecuredDebt", "user_reducer", false),
  })
  ...
  const questions = q.flat().map(d => ({ ...d, ...data[d.data] }))
